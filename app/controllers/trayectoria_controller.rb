class TrayectoriaController < ApplicationController
  def index
    @years = Post.select("DISTINCT strftime('%Y', published_at) as year").where('draft=? and section=? and published_at<?', false, :at, 30.days.ago).order('year desc').map{ |row| row.year }
